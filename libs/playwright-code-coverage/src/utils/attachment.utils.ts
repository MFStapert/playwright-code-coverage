...
export const marshallCoverage = (coverage: CoverageReport[]) => {
  return {
...
export const unmarshallCoverage = (attachments: Attachment[]) => {
  const coverageAttachment = attachments.find(a => a.name === coverageFileKey);
  if (coverageAttachment?.body) {
    return JSON.parse(coverageAttachment.body.toString()) as CoverageReport[];
  }
  return [];
};
